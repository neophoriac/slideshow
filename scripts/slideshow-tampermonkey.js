...
`
            ...
            this.imageContainer.addEventListener('wheel', e => {
                if (e.buttons === 1 && e?.wheelDelta > 0) {
                    this.rotateRecenter(1);
                } else if (e.buttons === 1 && e?.wheelDelta < 0) {
                    this.rotateRecenter(-1);
                } else {
                    this.zoom(e);
                }
            ...
            this.drag = (e, image) => {
                ...
                // will store last four pos values (difference between previous mouse position and current)
                ...
                function raf(e) {
                    if (!rafNeeded) { return };
                    rafNeeded = false;
                    requestAnimationFrame((timestamp) => { this.move(e) })
                }
            ...
            this.zoomToSelection = (selectionRect) => {
                ...
                const priorWidth = this.prevRect.width;
                const priorHeight = this.prevRect.height;

                // the pixels needed for the photo to be centered
                const priorCenterX = (this.containerRect.width - priorWidth) / 2;
                const priorCenterY = (this.containerRect.height - priorHeight) / 2;

                // how much the photo is shifted from the center
                const scrollCenterOffsetX = priorWidth > this.containerRect.width ? Math.abs(this.translateX) + priorCenterX : 0;
                const scrollCenterOffsetY = priorHeight > this.containerRect.height ? Math.abs(this.translateY) + priorCenterY : 0;
                ...
                let maxRatio = 1;
                const maxWidthRatio = this.containerRect.width / selectionRect.width;
                const maxHeightRatio = this.containerRect.height / selectionRect.height;
                ...
                const scaleDiff = (this.scale / priorScale);
                ...
                const imageSizeDiffX = (this.containerRect.width - priorWidth) / 2;
                const imageSizeDiffY = (this.containerRect.height - priorHeight) / 2;

                // the left and top values of our selection within the container
                const selectionleft = selectionRect.left - this.containerRect.left;
                const selectionTop = selectionRect.top - this.containerRect.top;
            ...
            this.positionImage = (rect = null) => {
                ...
                const center = this.rotatedCoords(0, 0, this.rotate);
                ...
                const scaleOffsetX = (this.initialRect.width * this.scale - this.initialRect.width) / 2;
                const scaleOffsetY = (this.initialRect.height * this.scale - this.initialRect.height) / 2;
                ...
                const centerX = (this.containerRect.width - rect.width) / 2;
                const centerY = (this.containerRect.height - rect.height) / 2;
                ...
                const left = rect.width < this.containerRect.width ? scaleOffsetX + centerX + rotatedDiffCx : scaleOffsetX + rotatedDiffCx;
                const top = rect.height < this.containerRect.height ? scaleOffsetY + centerY + rotatedDiffCy : scaleOffsetY + rotatedDiffCy;
            ...
            this.rotateRecenter = (rotation) => {
                ...
                rotation *= this.flipX * this.flipY;

                if (rotation === 1) { // if positive rotation
                    this.rotate = Math.min(360, this.rotate + this.degInterval);
                    rotateDir = 1 * this.flipX * this.flipY;
                } else { // if negative rotation
                    this.rotate = Math.max(-360, this.rotate - this.degInterval);
                    rotateDir = -1 * this.flipX * this.flipY;
                }
                ...
                const pointRotation = this.rotatedCoords(viewCenterX - rect.width / 2, viewCenterY - rect.height / 2, this.degInterval * rotateDir); // we rotate it around the center of the image
                ...
                this.translateX = newPositionX;
                this.translateY = newPositionY;
            ...
            this.rotatedCoords = (x, y, deg) => {
                ...
                const theta = 360 - deg === 360 ? 0 : 360 - deg

                const X = x * Math.cos(this.toRadians(theta)) - y * Math.sin(this.toRadians(theta));
                const Y = x * Math.sin(this.toRadians(theta)) + y * Math.cos(this.toRadians(theta));

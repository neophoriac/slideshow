...
`
        ...
        this.drag = (e, container) => {
            ...
            e.preventDefault();
            window.addEventListener("mousemove", raf);
            ...
            function raf(e){
                requestAnimationFrame((timestamp)=>{move(e, timestamp)})
            }
            ...
            function move(e, timestamp) {
                ...
                container.scrollBy(posX, posY);

                if (samples.length == 4) {
                    samples.shift();
                }
                samples.push({ x: container.scrollLeft, y: container.scrollHeight - container.scrollTop, ts: performance.now() });
            }
